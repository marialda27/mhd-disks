...
vex=ve$x[which(rmin>re)]
vey=ve$y[which(rmin>re)]
vez=ve$z[which(rmin>re)]
modve2=vex*vex + vey*vey + vez*vez

masse=masse[which(rmin>re)]
...
vcx=vc$x[which(rmin>rc)]
vcy=vc$y[which(rmin>rc)]
vcz=vc$z[which(rmin>rc)]
modvc2=vcx*vcx + vcy*vcy + vcz*vcz

massc=massc[which(rmin>rc)]
...
title("Kinetic energy in the cavity")
dev.off()
