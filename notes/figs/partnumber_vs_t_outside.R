...
pdf("partnumber_vs_t_outside.pdf")
...
rhoes = rhoe[which(rmin<re & ze>1)]
rhoedisco=length(rhoes)
rhocs = rhoc[which(rmin<rc & zc>1)]
...
plot(seq(0,350,10),rhoemedianai,log="y",xlab="time",
     ylab=expression(n[o]/n[x]),yaxt="n",type="b",pch=21,col="blue",
...
title("Number of particles outside disc/Initial or Current total number")
dev.off()
