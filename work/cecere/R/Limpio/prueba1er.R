...
xp = 10.*(runif(Npart) - 0.5)
yp = 10.*(runif(Npart) - 0.5)
zp = 10.*(runif(Npart) - 0.5)
...
for(il in 1:nlines)
{
  xl = 1.#runif(1)*xlong  #donde quiero calcular el campo, ahora un numero random
  yl = 3.#runif(1)*ylong
  zl = 0.0001#runif(1)*zlong
#   bold=data.frame(bx=0.0,by=0.0,bz=0.0)
