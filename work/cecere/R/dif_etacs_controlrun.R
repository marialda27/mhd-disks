...
library('snapshot')
idcontrol=snap.read.2('/home/cecere/sshfs/is2/fstasys/MHD_Disc/controlrun/snapshot_350','ID  ',debug=1,gas=1)
idetacs=  snap.read.2('/home/cecere/sshfs/is2/fstasys/MHD_Disc/etacs/snapshot_350','ID  ',debug=1,gas=1)

# PARA VER LO QUE LE FALTA A CONTROL
where <- match( idetacs, idcontrol )
# where <- match( idetacst0_ord, idetacs_ord )
where[ is.na( where ) ] <- 0

aca <- which(where == 0)
idetacs[aca]  #son las particulas de etacs que no estan en control

# voy tomar algunas particulas, pq son muchas
idsam <- sample(idetacs[aca], 1000, replace=FALSE)
...
# PARA VER LO QUE LE FALTA A CONTROL
where <- match( idcontrol, idetacs )
# where <- match( idetacst0_ord, idetacs_ord )
where[ is.na( where ) ] <- 0

aca <- which(where == 0)
idcontrol[aca]  #son las particulas de control que no estan en etacs

# voy tomar algunas particulas, pq son muchas
idsam <- sample(idcontrol[aca], 1000, replace=FALSE)
