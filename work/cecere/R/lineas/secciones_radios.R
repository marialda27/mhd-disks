...
# aca construyo un array con los valores de 
# datapos=NULL
...
possam <- sample(possel, 10000, replace=FALSE)
...
# plot(x,y,xlim=c(-5,5),ylim=c(-5,5),type = "o", col = "red")
# plot(x,z,xlim=c(-5,5),ylim=c(-5,5),type = "o", col = "red")
...
save(possam,cx,cy,cz,file="disco_2r3")
# print(load("disco_4r6"))
...
for (it in 1:length(rr)){
  plot(sqrt(cx[,it]*cx[,it]+cy[,it]*cy[,it]), cz[,it],xlim=c(0,50),ylim=c(-35,35))
  lines(c(0,15),c(2,2))
  lines(c(2,2),c(-30,30))
  lines(c(0,15),c(-2,-2))
...
save(possam,xx,yy,zz,B2,K,v2,file="disco_2r3")
print(load("z30t180dt10T350"))
