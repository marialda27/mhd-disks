...
pos=snap.read.2('/home/cecere/sshfs/is2/fstasys/MHD_Disc/etacs/snapshot_130','POS ',debug=1,gas=1)
xp = pos$x
yp = pos$y
zp = pos$z

vel=snap.read.2('/home/cecere/sshfs/is2/fstasys/MHD_Disc/etacs/snapshot_130','VEL ',debug=1,gas=1)
vz = vel$z

id=snap.read.2('/home/cecere/sshfs/is2/fstasys/MHD_Disc/etacs/snapshot_130','ID  ',debug=1,gas=1)

data <- data.frame(id,xp,yp,zp)

# me da el orden del array donde pasa esto
possel <- which(vz < 0 & 9.5 < pos$z & pos$z < 10.5 & -20 < pos$x & pos$x < 20 & -20 < pos$y & pos$y < 20) 

plot(pos$x[possel],pos$y[possel])

datapos=NULL
for (i in 1:length(possel)){
 datapos=c(datapos,possel[i])
}

# esto me da un muestreo de las posiciones del array
possam <- sample(datapos, 500, replace=FALSE)
...
# save(possam,cx,cy,cz,file="z10t180dt10T350")
# print(load("z10t180dt10T350"))

save(possam,cx,cy,cz,file="z10t130dt10T350")
print(load("z10t130dt10T350"))
...
for (it in 1:length(rr)){
  plot(sqrt(cx[,it]*cx[,it]+cy[,it]*cy[,it]), cz[,it],xlim=c(0,50),ylim=c(0,35))
  lines(c(0,15),c(2,2))
  lines(c(2,2),c(0,30))
...
save(possam,cx,cy,cz,file="z30t180dt10T350")
print(load("z30t180dt10T350"))
