...
for(i in 1:length(pos$x))   #lazo sobre todas las particulas size(ll)=Npart
{
# para cada posicion del la particula i (pos$x[i],pos$y[i],pos$z[i])
# 0 < x <longx ; calculo en que celda cae:
  ix=floor(pos$x[i]/deltax)+1    #1<=ix<=nx
  if(ix>nx)ix=nx
  iy=floor(pos$y[i]/deltay)+1    #1<=iy<=ny
  if(iy>ny)iy=ny
  iz=floor(pos$z[i]/deltaz)+1    #1<=iz<=nz
  if(iz>nz)iz=nz
  lirst[ix,iy,iz]=i
}
...
{
  ix=floor(pos$x[i]/deltax)+1 
  if(ix>nx)ix=nx
  iy=floor(pos$y[i]/deltay)+1
  if(iy>ny)iy=ny
  iz=floor(pos$z[i]/deltaz)+1    
  if(iz>nz)iz=nz
